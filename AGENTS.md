...
- **로컬 Keycloak 스택 ✅(2026-06-26, 코드)**: [docker-compose.keycloak.yml](docker-compose.keycloak.yml) 추가(Keycloak + 전용 Postgres, `start-dev --import-realm`), [infra/keycloak/import/eumgil-realm.json](infra/keycloak/import/eumgil-realm.json) 추가(`eumgil` realm, confidential client `eumgil-web`, secret `eumgil-local-dev-secret`, demo/admin-user 테스트 계정), [infra/keycloak/README.md](infra/keycloak/README.md) 추가. 루트 스크립트 `keycloak:up/down/logs/reset` + `verify:keycloak` 추가. 공식 Keycloak container 문서 기준: dev mode 는 로컬 검증용이고 운영에 쓰면 안 됨. 현재 환경에서는 Docker daemon 미실행으로 실제 기동은 미검증(`Cannot connect to the Docker daemon`).
- **다음 코딩 작업**: 로컬 Keycloak 실제 기동 검증(`pnpm keycloak:up`) + Auth.js 로그인 플로우 확인, Google→Kakao→Naver 순으로 Keycloak Identity Provider 브로커링 검증, Keycloak end-session endpoint 로 SSO 로그아웃 연결, 보호 라우트 세션 가드(저장/리뷰/설정 등 접근 정책 확정), 리뷰/방문 쓰기 Server Action/Form, 스팟 북마크 UX를 테마 저장 모델과 맞추기(현재 스팟 상세 북마크는 클라이언트 로컬 표시).
