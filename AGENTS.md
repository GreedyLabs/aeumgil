...
- **테스트 ①②완료**: 순수 함수(matching/scoring/grid/congestion/course-reorder/course-compose/travel-time) + API 정규화(kma/airkorea/tourapi)·LiveRepository 폴백/DB 코스 카탈로그 배선·캐시·에이전트·user-data·course-catalog·course route 옵션 전달 = **97개 통과**(2026-06-30 `corepack pnpm --filter @eumgil/web test` green). `corepack pnpm --filter @eumgil/web typecheck` 통과, `corepack pnpm --filter @eumgil/web build` 통과(빌드 중 일부 공공 API는 네트워크 fallback 로그 발생, 빌드 성공).
...
- **코스 생성 MVP ✅(2026-06-29 → 2026-06-30 UI·동선 갱신)**: [course-compose.ts](apps/web/src/domain/course-compose.ts) 순수함수 추가. 기존 큐레이션 코스는 고정 결과가 아니라 시간 슬롯 템플릿으로 사용하고, 테마/POI/식당/숙소/대체지 후보 풀을 받아 혼잡한 스팟을 같은 테마·권역 대체 후보로 교체, 식당·숙소는 당일 스팟 권역에 맞춰 재선택한다. [travel-time.ts](apps/web/src/domain/travel-time.ts) 순수 함수가 좌표 기반 도로 이동시간을 근사하고, composer 는 원래 장소와의 거리뿐 아니라 하루 전후 방문지를 기준으로 추가 이동분이 큰 대체 후보를 낮춘다. baseCourse 없이 새 코스를 만들 때도 연속 이동시간이 긴 후보를 낮춘다. `days/pace/companion/startRegion` 조건을 받아 일수·하루 스팟 수·가족/권역 우선순위를 조정한다. `/course/[id]`는 동일 이름의 쿼리 파라미터를 Repository 선택 인자로 전달하고, [course.tsx](apps/web/src/components/screens/course.tsx)에 조건 패널(일정/페이스/동행/시작 권역) UI가 추가되어 URL 갱신→서버 Repository 재호출로 코스를 다시 생성한다. `LiveRepository.getCourse()`는 `composeSeedCourse()`로 시드/DB 기반 코스를 만든 뒤 `planItinerary` 혼잡 재정렬 에이전트에 넘긴다. 테스트 [course-compose.test.ts](apps/web/src/domain/course-compose.test.ts)(8개)·[travel-time.test.ts](apps/web/src/domain/travel-time.test.ts)(3개) 추가, [agent-wiring.test.ts](apps/web/src/data/live/agent-wiring.test.ts)는 “정적 refId 보존”이 아니라 “항목 수 유지 + 실제 후보 refId” 기준으로 갱신.
- **코스 생성 DB 테이블화 ✅(2026-06-29)**: [schema.ts](apps/web/src/server/db/schema.ts)에 `spot_profile`/`course_template`/`course_template_item` 추가. [apply-app-schema.mjs](apps/web/scripts/apply-app-schema.mjs)와 [verify-db.mjs](apps/web/scripts/verify-db.mjs)도 새 테이블을 생성·검증하도록 갱신. [course-catalog.ts](apps/web/src/server/db/course-catalog.ts)는 DB `course_template`을 도메인 `Course`로, DB `spot_profile`을 도메인 `Spot`으로 정규화한다. `LiveRepository.composeSeedCourse()`는 DB 템플릿/스팟 후보가 있으면 우선 사용, 없거나 실패하면 기존 시드 템플릿/후보로 폴백한다. [seed-course-catalog.mjs](apps/web/scripts/seed-course-catalog.mjs) 추가 + `pnpm --filter @eumgil/web db:seed:course` 등록: mock `DATA.SPOTS/COURSES`와 `spot-mapping.ts` 좌표/env를 `spot_profile/course_template/course_template_item`으로 upsert. 테스트 [course-catalog.test.ts](apps/web/src/server/db/course-catalog.test.ts)(6개) + [course-catalog-wiring.test.ts](apps/web/src/data/live/course-catalog-wiring.test.ts)(4개)로 DB 템플릿 우선/DB 후보 우선/시드 폴백/옵션 반영을 검증.
...
- **다음 코딩 작업**: 실제 길찾기 API 기반 이동시간 포트 연결, Phase 5 품질(빌드/실 DB·실 로그인 회귀 확인, 접근성·반응형 점검), 또는 조건 입력을 Agent 도구 인자로 확장. Google→Kakao→Naver Keycloak Identity Provider 브로커링은 후순위로 보류.
