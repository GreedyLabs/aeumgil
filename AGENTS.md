...
- **테스트 ①②완료**: 순수 함수(matching/scoring/grid/congestion/course-reorder/course-compose/travel-time) + API 정규화(kma/airkorea/tourapi)·LiveRepository 폴백/DB 코스 카탈로그 배선·캐시·에이전트·user-data·course-catalog·course route 옵션 전달·서버 이동시간 포트 = **104개 통과**(2026-06-30 `corepack pnpm --filter @eumgil/web test` green). `corepack pnpm --filter @eumgil/web typecheck` 통과, `corepack pnpm --filter @eumgil/web build` 통과(빌드 중 일부 공공 API는 네트워크 fallback 로그 발생, 빌드 성공).
...
- **다음 코딩 작업**: 실제 길찾기 API 어댑터 선택·키 검증 후 이동시간 포트에 연결, Phase 5 품질(실 DB·실 로그인 회귀 확인, 접근성·반응형 점검), 또는 조건 입력을 Agent 도구 인자로 확장. Google→Kakao→Naver Keycloak Identity Provider 브로커링은 후순위로 보류.
