...
- **테스트 ①②완료**: 순수 함수(matching/scoring/grid/congestion/course-reorder/course-compose) + API 정규화(kma/airkorea/tourapi)·LiveRepository 폴백/DB 코스 카탈로그 배선·캐시·에이전트·user-data·course-catalog·course route 옵션 전달 = **92개 통과**(2026-06-29 `corepack pnpm --filter @eumgil/web test` green). `corepack pnpm --filter @eumgil/web typecheck` 통과, `corepack pnpm --filter @eumgil/web build` 통과(빌드 중 일부 공공 API는 네트워크 fallback 로그 발생, 빌드 성공).
...
- **코스 생성 MVP ✅(2026-06-29)**: [course-compose.ts](apps/web/src/domain/course-compose.ts) 순수함수 추가. 기존 큐레이션 코스는 고정 결과가 아니라 시간 슬롯 템플릿으로 사용하고, 테마/POI/식당/숙소/대체지 후보 풀을 받아 혼잡한 스팟을 같은 테마·권역 대체 후보로 교체, 식당·숙소는 당일 스팟 권역에 맞춰 재선택한다. 좌표가 있는 후보는 원래 장소와의 직선거리 패널티를 적용해 너무 먼 대체지를 낮춘다(길찾기 API 전 단계 근사). `days/pace/companion/startRegion` 조건을 받아 일수·하루 스팟 수·가족/권역 우선순위를 조정한다. `/course/[id]`는 동일 이름의 쿼리 파라미터를 Repository 선택 인자로 전달한다. baseCourse 가 없어도 후보 풀에서 간단한 코스를 생성 가능. `LiveRepository.getCourse()`는 `composeSeedCourse()`로 시드/DB 기반 코스를 만든 뒤 `planItinerary` 혼잡 재정렬 에이전트에 넘긴다. 테스트 [course-compose.test.ts](apps/web/src/domain/course-compose.test.ts)(6개) 추가, [agent-wiring.test.ts](apps/web/src/data/live/agent-wiring.test.ts)는 “정적 refId 보존”이 아니라 “항목 수 유지 + 실제 후보 refId” 기준으로 갱신.
...
- **다음 코딩 작업**: 실제 도로 이동시간 기반 랭킹, 코스 조건 입력 UI, 또는 Phase 5 품질(빌드/실 DB·실 로그인 회귀 확인, 접근성·반응형 점검). Google→Kakao→Naver Keycloak Identity Provider 브로커링은 후순위로 보류.
