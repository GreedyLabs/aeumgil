...
export function LoginView({
  ...
  return (
    <div className="login-page screen-enter">
      <UI.TopBar title={translateUi("로그인")} />
      <div className="login-layout">
        <section className="login-story">
          <div>
            <span className="eyebrow">{translateUi("나의 강원 여행")}</span>
            <h1>{translateUi("여행을 이어가요")}</h1>
            <p>
              {translateUi("한적한 바다부터 초록빛 숲길까지.")}
              <br />
              {translateUi("가고 싶은 곳과 다녀온 순간을 모아보세요.")}
            </p>
          </div>
          <div className="login-landscape" aria-hidden="true" />
          <div className="login-story-caption">
            <Icon.pin />
            {translateUi(" 강원에서 찾은, 나만의 에움길")}
          </div>
        </section>
        <section className="login-card" aria-labelledby="login-title">
          <span className="eyebrow">{translateUi("어서 오세요")}</span>
          <h2 id="login-title">{translateUi("여행을 이어갈까요?")}</h2>
          <p className="login-description">
            {translateUi("계정 하나로 여행 코스와 기록을")}
            <br className="desktop-only" />
            {translateUi(" 여러 기기에서 함께 볼 수 있어요.")}
          </p>
          {reasonCopy && (
            <p className="login-reason">
              <Icon.bookmark />
              {translateUi(reasonCopy)}
            </p>
          )}
          <ul className="login-benefits">
            <li>
              <Icon.bookmark />
              <span>{translateUi("마음에 드는 테마와 코스 저장")}</span>
            </li>
            <li>
              <Icon.pin />
              <span>{translateUi("방문 기록과 나만의 리뷰")}</span>
            </li>
            <li>
              <Icon.leaf />
              <span>{translateUi("관심 테마와 여행 취향 설정")}</span>
            </li>
          </ul>
          {failed && (
            <p className="login-error" role="alert">
              {translateUi("로그인을 연결하지 못했어요. 잠시 후 다시 시도해 주세요.")}
            </p>
          )}
          <div className="login-actions">
            ...
            {!providers.length && (
              <p role="status">
                {translateUi("로그인을 준비하고 있어요. 여행지는 바로 둘러볼 수 있어요.")}
              </p>
            )}
            <Link className="btn btn-secondary btn-block" href="/discover">
              {translateUi("먼저 여행지 둘러보기")}
            </Link>
          </div>
          <p className="login-account-hint">
            {translateUi("처음 오셨나요? 다음 화면에서 회원가입할 수 있어요.")}
          </p>
