...
export function CourseMap({
    ...
    void import("leaflet")
      .then((L) => {
        ...
        stops.forEach((stop, i) => {
          ...
          time.textContent = `${stop.time} 방문 예정`;
          popup.append(time);
  ...
  return (
    <section className="course-route-map card" aria-label={`Day ${day} 동선 지도`}>
      <div className="section-heading">
        <div>
          <span className="eyebrow">Day {day} · 방문 순서와 이동</span>
          <h2>하루 동선 한눈에</h2>
        </div>
        <span className="route-total">
          {stops.length}곳{total > 0 ? ` · 이동 약 ${total}분` : ""}
        </span>
      </div>
