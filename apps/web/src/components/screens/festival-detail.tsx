      ...
      <div className="festival-detail-content">
        <header className="festival-detail-header">
          <span className="eyebrow">
            {translateUi(localized(place.region, lang))} · {translateUi(status)}
          </span>
          <h1>{translateUi(localized(event.name, lang))}</h1>
        ...
        {place.imageUrl && (
          <div className="festival-hero">
            <UI.Placeholder
              label={translateUi(localized(event.name, lang))}
              src={place.imageUrl}
              h={330}
              priority
            />
            <span className="data-caption">{translateUi("사진 제공: 한국관광공사")}</span>
          </div>
        )}
        <div className="festival-info-layout">
          <section className="card public-info-card">
            <h2>{translateUi("행사를 만나기 전에")}</h2>
            <dl className="visit-facts">
              {facts.map(([label, value]) => (
                <div key={label}>
                  <dt>{translateUi(label)}</dt>
                  <dd>{translateUi(value)}</dd>
                </div>
              ))}
            </dl>
            {detail.homepage && (
          ...
          <section className="card public-info-card">
            <h2>{translateUi("행사장 위치")}</h2>
            {place.lat !== undefined && place.lon !== undefined ? (
              <LocationMap lat={place.lat} lon={place.lon} name={localized(event.name, lang)} />
            ) : (
              <p>{translateUi("등록된 좌표가 없어요. 행사장 주소를 확인해 주세요.")}</p>
            )}
            <p>{translateUi(detail.venue || event.address)}</p>
          </section>
        </div>
        <TranslationNotice translation={place.translation} />
        {place.description?.ko && (
          <section className="card public-info-card">
            <h2>{translateUi("어떤 행사인가요?")}</h2>
            <p
            ...
            {longDescription && (
              ...
              >
                {translateUi(expanded ? "소개 접기" : "소개 더 보기")} <Icon.chevD />
              </button>
            )}
          </section>
        )}
        ...
        <section className="festival-nearby">
          <div className="section-heading">
            <h2>{translateUi("함께 들를 여행지")}</h2>
            <span className="data-caption">{translateUi("행사장 반경 10km 이내")}</span>
          </div>
          <div className="commerce-grid">
            {detail.nearbySpots.slice(0, 4).map((spot) => (
              <article className="card festival-nearby-card" key={spot.id}>
                <Link href={`/spot/${spot.id}`}>
                  <UI.Placeholder label={localized(spot.name, lang)} src={spot.imageUrl} h={160} />
                </Link>
                <div className="commerce-body">
                  <h3>{localized(spot.name, lang)}</h3>
                  <p>{translateUi(spot.address)}</p>
          ...
          {!detail.nearbySpots.length && (
            <p>{translateUi("가까운 등록 여행지가 없어요. 탐색에서 다른 장소를 찾아보세요.")}</p>
          )}
        </section>
        <section className="festival-nearby">
          <div className="section-heading">
            <h2>{translateUi("근처에서 식사해요")}</h2>
            <span className="data-caption">{translateUi("행사장 반경 10km 이내")}</span>
          </div>
          <div className="commerce-grid">
            {detail.nearbyEats.slice(0, 4).map((eat) => (
              <div className="festival-food-card" key={eat.id}>
                <NearbyPlaceCard
                  imageLabel={localized(eat.name, lang)}
                  imageUrl={eat.imageUrl}
                  address={eat.address}
                  title={localized(eat.name, lang)}
                  category={localized(eat.type, lang)}
                  meta=""
                  tone="accent"
                  icon="utensils"
                />
                ...
                >
                  {translateUi("행사와 식당을 코스에 담기")}
                </Link>
              </div>
            ))}
          </div>
          {!detail.nearbyEats.length && <p>{translateUi("가까운 등록 음식점 정보가 없어요.")}</p>}
        </section>
      </div>
    </div>
  );
