"use client";
import { formatObservationTime } from "@/lib/detail-screen-messages";
...
function ObservationTime({ value }: { value: string }) {
  ...
  return <time dateTime={value}>{formatObservationTime(value, lang)}</time>;
}
  ...
  return (
    <li className={styles.segment}>
      <div className={styles.segmentBody}>
        <strong>{translateUi(segment.name.replace(/\s*[-–]\s*/g, " → "))}</strong>
        <small>
          <ObservationTime value={segment.observedAt} />
          {translateUi(" 관측")}
        </small>
      </div>
      <div className={`${styles.speed} ${styles[level]}`}>
        <span>{translateUi(speedLabels[level])}</span>
        <strong>
          {translateUi(segment.speed)}
          <small> km/h</small>
        </strong>
      </div>
    </li>
...
function RoadCard({ road }: { road: HighwayStatus }) {
  ...
  return (
    <article className={styles.road} aria-label={translateUi(road.route)}>
      <header className={styles.roadHeader}>
        <span
          className={styles.routeNumber}
          aria-label={translateUi(`고속도로 ${route.number}호선`)}
        >
          {translateUi(route.number)}
        </span>
        <div>
          <h3>{translateUi(road.route)}</h3>
          {"scope" in route && <small>{translateUi(route.scope)}</small>}
        </div>
      </header>
      <div className={styles.roadStatus}>
        {road.status === "current" ? (
          <>
            ...
            <small>
              {translateUi(road.segments)}
              {translateUi("개 구간·방향 관측")}
            </small>
          </>
        ) : (
          <>
            <strong>
              {translateUi(road.status === "stale" ? "최근 관측 갱신 지연" : "현재 관측 정보 없음")}
            </strong>
            <small>
              {translateUi(
                road.status === "stale"
      ...
      {road.status === "current" && (
        <p className={styles.breakdown}>
          <span>
            ...
            {translateUi("40 미만 ")}
            <b>{translateUi(road.slowSegments)}</b>
          </span>
          <span>
            <i className={styles.barDelayed} />
            40–79 <b>{translateUi(road.delayedSegments)}</b>
          </span>
          <span>
            <i className={styles.barClear} />
            {translateUi("80 이상 ")}
            <b>{translateUi(clear)}</b>
          </span>
        </p>
      )}
      {alerts.length > 0 && (
        <>
          ...
          {alerts.length > 3 && (
            <details className={styles.more}>
              <summary>
                {translateUi("80km/h 미만 ")}
                {translateUi(alerts.length - 3)}
                {translateUi("구간 더 보기")}
              </summary>
              <ul className={styles.segmentList}>
      ...
      {road.observedAt && (
        <p className={styles.observed}>
          {translateUi(road.status === "stale" ? "마지막 관측 " : "관측 기준 ")}
          <ObservationTime value={road.observedAt} />
        </p>
      )}
      {unknown > 0 && (
        <p className={styles.observed}>
          {translateUi("현재 판단 제외: ")}
          {translateUi(road.staleSegments > 0 && `갱신 지연 ${road.staleSegments}구간`)}
          {translateUi(road.staleSegments > 0 && road.unavailableSegments > 0 && " · ")}
          {translateUi(road.unavailableSegments > 0 && `미측정 ${road.unavailableSegments}구간`)}
        </p>
      )}
    </article>
  );
  ...
  return (
    ...
    >
      <div className={styles.heading}>
        <div>
          <div className="section-label">{translateUi("출발 전, 돌아오는 길에도")}</div>
          <h2 id="home-traffic-title" className="section-title">
            {translateUi("어느 구간에서 속도가 줄어들까요?")}
          </h2>
          <p>{translateUi("강원과 연결되는 세 고속도로의 저속 구간부터 확인하세요.")}</p>
        </div>
        <ExternalLink className="text-link" href="https://www.roadplus.co.kr/">
          {translateUi("도로공사 교통 지도")}
        </ExternalLink>
      </div>
      ...
      <details className={styles.explanation}>
        <summary>{translateUi("속도와 구간 표시 기준")}</summary>
        <p>
          {translateUi(
      ...
      <p className={styles.source}>
        {translateUi("한국도로공사 제공 · 구간·방향별 최저 관측속도 · 막대는 구간 수 비율")}
      </p>
    </section>
  );
}
...
export function TrafficOverviewLoading() {
  ...
  return (
    <section
      className={`page-section ${styles.overview}`}
      aria-label={translateUi("고속도로 교통 정보 불러오기")}
      aria-busy="true"
    >
      <h2 className="section-title">{translateUi("출발 전 고속도로 확인")}</h2>
      <p role="status">{translateUi("구간별 관측 정보를 확인하고 있어요…")}</p>
    </section>
  );
}
