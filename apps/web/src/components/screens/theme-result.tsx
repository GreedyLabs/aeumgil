...
export function ThemeResultView({
  ...
  return (
    <div className="screen-enter theme-page">
      <UI.TopBar
        title={translateUi(query ? "맞춤 테마 추천" : "테마 소개")}
        onBack={back}
        right={
          <button
            className="icon-btn"
            aria-label={translateUi("테마 공유")}
      ...
      <header className="page-heading">
        <div>
          <span className="eyebrow">
            {translateUi(query ? "✦ 여행의 기분과 가장 가까운 테마" : localized(theme.tag, lang))} ·
            {translateUi(" ")}
            {translateUi(localized(theme.region, lang))}
          </span>
          <h1>{translateUi(localized(theme.title, lang))}</h1>
          <p>{translateUi(localized(theme.subtitle, lang))}</p>
        </div>
      </header>
      {query && <blockquote className="query-quote">“{translateUi(query)}”</blockquote>}
      <div className="detail-layout">
        <div className="detail-primary">
          ...
          <div className="theme-intro">
            <p>{translateUi(localized(theme.blurb, lang))}</p>
            <div className="mood-list">
              {theme.mood.map((m) => (
                <UI.Chip key={m.ko} brand>
                  {translateUi(localized(m, lang))}
                </UI.Chip>
              ))}
            </div>
          </div>
          <section className="section-block">
            <h2>{translateUi("이 여행에서 만나는 곳")}</h2>
        ...
        <aside
          ...
          aria-label={translateUi("여행 요약")}
        >
          <details className="trip-facts">
            <summary>
              {translateUi(options.days ? `${options.days}일` : localized(theme.duration, lang))} ·
              {translateUi(" ")}
              {translateUi(localized(theme.region, lang))} <span>{translateUi("여행 요약")}</span>
            </summary>
            <dl className="summary-facts">
              <div>
                <dt>{translateUi("여행 지역")}</dt>
                <dd>{translateUi(localized(theme.region, lang))}</dd>
              </div>
              <div>
                <dt>{translateUi("여행 일정")}</dt>
                <dd>
                  {translateUi(
              ...
              <div>
                <dt>{translateUi("여행 분위기")}</dt>
                <dd>{translateUi(localized(theme.pace, lang))}</dd>
              </div>
              {options.transport && (
                <div>
                  ...
                  <dd>
                    {translateUi(
                      { car: "자동차", transit: "대중교통", walk: "도보" }[options.transport],
                    )}
                  </dd>
                </div>
              )}
      ...
      {alts.length > 0 && (
        <section className="section-block">
          <div className="section-heading">
            <div>
              <span className="eyebrow">{translateUi("다른 테마도 볼까요")}</span>
              <h2 className="section-title">{translateUi("비슷한 분위기")}</h2>
            </div>
          </div>
